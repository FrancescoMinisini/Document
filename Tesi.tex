...
% \usepackage{tikz}
% \usetikzlibrary{positioning,arrows.meta,fit}
\section{The Universal Quantum Control Framework}
...
\section{Empirical assessment of the control framework}
...
\end{equation}
while the nominal control cost is evaluated through the same UFO functional already introduced in Eq.~\eqref{eq:ufo_cost_function}. For robustness studies, deterministic evaluation is supplemented by Monte Carlo sampling of noisy control realizations. In that setting, three quantities are reported: the average channel fidelity estimator used in the implementation, the average gate fidelity over noisy realizations, and the fidelity variance. Denoting a noisy realization of the control trajectory by \(\mathcal{P}_{\xi}\), one may write schematically
\begin{equation}
...
As a first validation step, the framework was tested on a single representative target gate within the family \(N(\alpha,\alpha,\gamma)\), choosing
\begin{equation}
\alpha = 2.2,
\qquad
\gamma = \pi/2.
\label{eq:single_target_representative}
\end{equation}
This choice serves as a convenient benchmark for three reasons. First, it corresponds to a genuinely nontrivial two-qubit target, and therefore provides a more meaningful test than a gate too closely aligned with the identity or with a particularly simple native interaction. Second, it belongs to the same target family used throughout the original framework, which makes it suitable for validating the numerical implementation against the methodology discussed in the previous chapter. Third, the same target was adopted in the light hyperparameter sensitivity study of Section~\ref{subsec:experimental_protocol_setup}, so that the subsequent comparisons between Adam optimization, nominal TRPO, and noise-optimized TRPO are carried out under a consistent and well-calibrated setting. For these reasons, the gate in Eq.~\eqref{eq:single_target_representative} is used here as the main case study for assessing the nominal optimization behavior of the framework before moving to robustness and runtime analyses on broader families of targets.
