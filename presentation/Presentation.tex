...
\begin{block}{UFO cost function}
\[
C=
\underbrace{\chi(1-F)}_{\text{accuracy}}
+\underbrace{\beta L_{\mathrm{tot}}}_{\text{leakage}}
+\underbrace{\mu P_{\mathrm{bdry}}}_{\text{boundary}}
+\underbrace{\kappa T}_{\text{time}}
\]
\[
F=\frac{1}{16}\left|\mathrm{Tr}\!\left(U^\dagger(T)U_{\mathrm{target}}\right)\right|^2,
\qquad
...
\begin{block}{TSWT-based leakage proxy}
\[
L_{\mathrm{tot}}(T)\lesssim
\frac{\|\hat H_{\mathrm{od}}(0)\|}{\Delta(0)}
+
\frac{\|\hat H_{\mathrm{od}}(T)\|}{\Delta(T)}
+
\int_0^T
\frac{1}{\Delta(t)^2}
\left\|
\frac{d^2\hat H_{\mathrm{od}}(t)}{dt^2}
\right\|dt
\]
\end{block}
...
\begin{itemize}\itemsep2pt
    \item We must optimize \textbf{four competing goals}: fidelity, leakage suppression, pulse regularity, and runtime.
    \item The first two leakage terms are \textbf{boundary contributions}; the integral term penalizes \textbf{rapid pulse variations}.
\end{itemize}
