...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{chapters/plots/synthesis_vs_analog_control.pdf}
% \begin{tikzpicture}[
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{chapters/plots/weakly_anharmonic_ladder.pdf}
% \begin{tikzpicture}[
%     x=1cm, y=1cm,
%     level/.style={thick},
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/computational_vs_leakage_subspace.pdf}
% \begin{tikzpicture}[
%     box/.style={
%         draw,
%         rounded corners,
...
%     arrow/.style={-{Latex[length=2.2mm]}, thick},
%     internal/.style={-{Latex[length=2mm]}, thick},
%     node distance=1.8cm and 2.2cm
% ]

% \node[box] (comp) {
% $\mathcal{H}_{\mathrm{comp}}$\\[0.25em]
% $\ket{00},\ket{01},\ket{10},\ket{11},\dots$
% };

% \node[box, right=of comp] (leak) {
% $\mathcal{H}_{\mathrm{leak}}$\\[0.25em]
% $\ket{2},\ket{3},\dots$ locally\\
% or higher-excitation sectors
% };

% % Coupling between subspaces
...
% % Internal temporal evolution in computational subspace
% \draw[internal] 
%     ($(comp.north west)+(0.35,-0.55)$) -- ($(comp.north east)+(-0.35,-0.55)$);
% \node[tinylabel] at ($(comp.north)+(0,-0.28)$) {time evolution};
% \node[tinylabel] at ($(comp.south)+(0,0.40)$) {generated by $H_{\mathrm{diag}}(t)$};

% % Internal temporal evolution in leakage subspace
% \draw[internal] 
%     ($(leak.north west)+(0.35,-0.55)$) -- ($(leak.north east)+(-0.35,-0.55)$);
% \node[tinylabel] at ($(leak.north)+(0,-0.28)$) {time evolution};
% \node[tinylabel] at ($(leak.south)+(0,0.40)$) {generated by $H_{\mathrm{diag}}(t)$};
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{chapters/plots/speed_leakage_tradeoff.pdf}
% \begin{tikzpicture}[
%     x=1cm,y=1cm,
%     axis/.style={thick,->},
%     curve1/.style={thick},
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{chapters/plots/open_closed_loop_control.pdf}
% \begin{tikzpicture}[
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/robustness_landscape_shift.pdf}
% \begin{tikzpicture}[
%     axis/.style={thick,->},
%     curve/.style={thick},
%     lbl/.style={font=\small}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/rl_quantum_control_loop.pdf}
% \begin{tikzpicture}[
...
% \includegraphics[width=0.8\textwidth]{chapters/plots/continuous_action_trpo.pdf}
\tikzexternaldisable
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{chapters/plots/reward_design_bridge.pdf}
    %     \begin{tikzpicture}[
...
In the framework considered in this thesis, these approximators are implemented as feed-forward neural networks. 

\paragraph{Feed-forward neural network.}
A feed-forward neural network is a parametric nonlinear map built by composing a sequence of affine transformations and activation functions. Its defining structural feature is that information flows only in one direction, from the input layer to the output layer, without recurrent connections or feedback loops inside the network itself. The simplest building block is the \emph{perceptron}, which receives an input vector \(x=(x_1,\dots,x_n)\), forms a weighted sum with bias,
\begin{equation}
z = \sum_{i=1}^{n} w_i x_i + b = \bm{w}^{\top}\bm{x}+b,
\label{eq:perceptron_affine_map}
\end{equation}
and then applies a nonlinear activation function \(\phi\), yielding
\begin{equation}
y = \phi(z).
\label{eq:perceptron_output}
\end{equation}
A \emph{multilayer perceptron} (MLP) generalizes this idea by stacking many such units into successive hidden layers, so that the output of one layer becomes the input of the next. In this way, the network realizes a hierarchical nonlinear transformation of the form
\begin{equation}
h^{(1)} = \phi\!\left(W^{(1)}x+b^{(1)}\right), \qquad
h^{(2)} = \phi\!\left(W^{(2)}h^{(1)}+b^{(2)}\right), \qquad \dots
\label{eq:mlp_hidden_layers}
\end{equation}
up to a final output map
\begin{equation}
f_{\theta}(x)=W^{(L)}h^{(L-1)}+b^{(L)},
\label{eq:mlp_output_map}
\end{equation}
where \(\theta=\{W^{(\ell)},b^{(\ell)}\}_{\ell=1}^{L}\) denotes the full set of trainable parameters. The practical importance of this architecture is that the repeated alternation of linear maps and nonlinear activations allows the network to approximate highly nontrivial input--output relations while still remaining a finite-dimensional differentiable model. 
Given a differentiable objective function \(\mathcal{L}(\theta)\), the network parameters can then be updated by gradient-based optimization, with the required derivatives \(\nabla_{\theta}\mathcal{L}\) computed efficiently through the backpropagation algorithm, which applies the chain rule recursively from the output layer back to the earlier layers \cite{Rumelhart1986,Goodfellow2016}.
In the present context, this makes feed-forward networks a natural choice for representing the policy and value functions: they are flexible enough to capture nonlinear control strategies, yet simple enough to remain fully compatible with the actor--critic and TRPO framework adopted in this thesis.
...
\centering
\includegraphics[width=\textwidth]{chapters/plots/singlelayer_perceptron.pdf}
% \begin{tikzpicture}[
%     x=0.9cm,y=0.9cm,
%     greennode/.style={circle, draw=green!60!black, top color=green!5, bottom color=green!20, minimum size=6mm, inner sep=0pt},
%     graynode/.style={circle, draw=gray!60!black, top color=gray!5, bottom color=gray!20, minimum size=7mm, inner sep=0pt},
%     orangenode/.style={rectangle, draw=orange!60!black, top color=orange!5, bottom color=orange!20, rounded corners, minimum width=8mm, minimum height=7mm},
%     rednode/.style={circle, draw=red!60!black, top color=red!5, bottom color=red!20, minimum size=6mm, inner sep=0pt},
%     arrow/.style={-{Latex[length=1.8mm]}, thick}
% ]
% % Inputs
% \node[greennode] (x1) at (0,1.2) {$x_1$};
% \node[greennode] (x2) at (0,0.0) {$x_2$};
% \node[greennode] (x3) at (0,-1.2) {$x_3$};

% % Sum and activation
% \node[graynode] (sum) at (1.8,0.0) {$\Sigma$};
% \node[orangenode] (phi) at (3.8,0.0) {$\phi$};
% \node[rednode] (y) at (5.6,0.0) {$y$};

% % Connections
% \draw[arrow] (x1) -- node[pos=0.4, above, sloped] {\scriptsize $w_1$} (sum);
% \draw[arrow] (x2) -- node[pos=0.4, above, sloped] {\scriptsize $w_2$} (sum);
% \draw[arrow] (x3) -- node[pos=0.4, below, sloped] {\scriptsize $w_3$} (sum);
% \draw[arrow] (sum) -- node[midway, above=6pt] {\scriptsize $z = \bm w^\top \bm x + b$} (phi);
% \draw[arrow] (phi) -- node[midway, above=6pt] {\scriptsize $y = \phi(z)$} (y);

% % Bias
% \draw[arrow] (1.8,-1.2) -- node[right, pos=0.2] {\scriptsize $b$} (sum);
% \end{tikzpicture}
\caption{A single perceptron computes an affine combination of the inputs and then applies a nonlinear activation function.}
\label{fig:single_perceptron}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.56\textwidth}
\centering
\includegraphics[width=\textwidth]{chapters/plots/feedforward_mlp.pdf}
% \begin{tikzpicture}[
%     x=0.85cm,y=0.85cm,
%     greennode/.style={circle, draw=green!60!black, top color=green!5, bottom color=green!20, minimum size=5mm, inner sep=0pt},
%     bluenode/.style={circle, draw=blue!60!black, top color=blue!5, bottom color=blue!20, minimum size=5mm, inner sep=0pt},
%     rednode/.style={circle, draw=red!60!black, top color=red!5, bottom color=red!20, minimum size=5mm, inner sep=0pt},
%     arrow/.style={-{Latex[length=1.3mm]}, thin, gray!60}
% ]
% % Layer spacing positions
% \def\xone{0}
% \def\xtwo{1.2}
% \def\xthree{2.4}
% \def\xfour{3.8}
...
% % Nodes with symbolic dots for width (length)
% % Input Layer
% \node[greennode] (i1) at (\xone, 1.5) {};
% \node[greennode] (i2) at (\xone, 0.6) {};
% \node at (\xone, -0.1) {$\vdots$};
% \node[greennode] (i3) at (\xone, -1.0) {};

% % Hidden Layer 1
% \node[bluenode] (h11) at (\xtwo, 1.8) {};
% \node[bluenode] (h12) at (\xtwo, 0.8) {};
% \node at (\xtwo, 0.1) {$\vdots$};
% \node[bluenode] (h13) at (\xtwo, -0.7) {};

% % Hidden Layer 2
% \node[bluenode] (h21) at (\xthree, 1.8) {};
% \node[bluenode] (h22) at (\xthree, 0.8) {};
% \node at (\xthree, 0.1) {$\vdots$};
% \node[bluenode] (h23) at (\xthree, -0.7) {};

% % Depth dots (horizontal)
% \node at (3.1, 0.55) {$\dots$};

% % Hidden Layer L
% \node[bluenode] (hL1) at (\xfour, 1.8) {};
% \node[bluenode] (hL2) at (\xfour, 0.8) {};
% \node at (\xfour, 0.1) {$\vdots$};
% \node[bluenode] (hL3) at (\xfour, -0.7) {};

% % Output Layer
% \node[rednode] (o1) at (\xfive, 0.5) {};
% \node[rednode] (o2) at (\xfive, -0.5) {};
...
% % Illustrative connections to/from depth dots
% \foreach \i in {1,2,3} {
%     \draw[arrow] (h2\i) -- (2.9, 0.55+\i*0.2);
%     \draw[arrow] (3.4, 0.55+\i*0.2) -- (hL\i);
% }
...
% % Decorative Braces
% % Vertical braces for input/output
% \draw[decorate, decoration={brace, mirror, amplitude=4pt}, thick] (-0.5, 1.6) -- (-0.5, -1.1) 
%     node [midway, left=6pt, align=right] {\small input\\ \small layer};
% \draw[decorate, decoration={brace, amplitude=4pt}, thick] (5.7, 0.6) -- (5.7, -0.6) 
%     node [midway, right=6pt, align=left] {\small output\\ \small layer};

% % Horizontal brace for hidden space
% \draw[decorate, decoration={brace, amplitude=6pt}, thick] (1.1, 2.3) -- (3.9, 2.3) 
%     node [midway, above=6pt] {\small hidden layers space};

% % Functional labels
% \node at (\xone,2.1) {\small $\bm{x}$};
% \node at (\xfive,1.2) {\small $f_\theta(\bm{x})$};

% \end{tikzpicture}
\caption{A feed-forward multilayer perceptron propagates information through successive hidden layers, thereby constructing a nonlinear map from input to output.}
\label{fig:feedforward_mlp}
\end{subfigure}

\caption{Elementary building blocks of feed-forward neural networks. The left panel shows the structure of a single perceptron, while the right panel illustrates a multilayer perceptron obtained by composing many perceptrons into successive hidden layers.}
\label{fig:perceptron_and_mlp}
\end{figure}

This choice should be understood in a pragmatic rather than purely algorithmic sense. The goal is not to introduce a separate layer of physical modeling based on machine learning, but to provide a flexible finite-dimensional representation for the two central objects of actor--critic reinforcement learning: the policy
...
\begin{figure}[htpb]
\centering
\includegraphics[width=\textwidth]{chapters/plots/actor_critic_nn_control.pdf}
% \begin{tikzpicture}[
