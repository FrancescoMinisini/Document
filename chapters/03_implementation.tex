...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/gmon_architecture_schematic.pdf}
% \begin{tikzpicture}[
...
% \includegraphics[width=0.8\textwidth]{chapters/plots/gmon_control_channels.pdf}
\tikzexternaldisable
\begin{figure}[t]
\centering
\begin{tikzpicture}[
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{chapters/plots/paper_subspace_structure.pdf}
% \begin{tikzpicture}[
%     x=1.0cm, y=1.0cm,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/tswt_motivation.pdf}
% \begin{tikzpicture}[
...
Starting from this residual Hamiltonian, as derived in Appendix~\ref{app:tswt_coherent_leakage_bound}, a practical upper bound for the accumulated coherent leakage is given by
\begin{equation}
L_{\mathrm{tot}}(T)
\lesssim
...
The incremental evaluation of the TSWT-based leakage proxy used during simulation is summarized in Algorithm~\ref{alg:tswt_leakage_compact}, while a more explicit version of this estimator, closer to the actual numerical implementation, is reported in Appendix~\ref{app:Incremental_TSWT}.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{chapters/plots/structure_leakage_bound.pdf}
% \begin{tikzpicture}[
...
\begin{algorithm}[t]
\caption{Incremental update of the TSWT-based leakage estimator}
\label{alg:tswt_leakage_compact}
\begin{algorithmic}[1]
\Require instantaneous Hamiltonian $H_t$, effective anharmonicity $\eta_t$
\State decompose $H_t$ into $H_0$, $H_1$, and $H_2$
\State compute the first TSWT generator $S_1$
\State estimate $\dot S_1$ from finite differences
\State compute the second TSWT generator $S_2$
\State estimate $\dot S_2$ from finite differences
\State construct the residual third-order block-off-diagonal term $H_{\mathrm{od}}^{(3)}$
\State evaluate its spectral norm and the instantaneous gap scale $\Delta_t$
\State update the boundary contribution using the first and last samples
\State update the integral contribution from the discrete second derivative of $H_{\mathrm{od}}^{(3)}$
\State store the current quantities for the next step
\State \Return updated leakage estimate
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{chapters/plots/ufo_cost_structure.pdf}
% \begin{tikzpicture}[
...
\begin{figure}[t]
\centering
% \includegraphics[width=0.8\textwidth]{chapters/plots/paper_rl_loop.pdf}
\begin{tikzpicture}[
...
The interaction between the policy and the quantum-control environment is summarized in Algorithm~\ref{alg:env_step_compact}, while, for completeness, a more explicit episode-level version of the rollout procedure is reported in Appendix~\ref{app:env_rollout}.
...
\caption{Single environment step in the quantum-control RL setting}
\label{alg:env_step_compact}
\begin{algorithmic}[1]
\Require current observation $o_t$, propagated unitary $U_t$, action $a_t$
\State map $a_t$ to physical controls $u_t$
\State apply the bandwidth filter to obtain $\tilde u_t$
\State sample noisy controls $\tilde u_t^{(\mathrm{noisy})}$ and effective anharmonicity $\eta_t$ if stochastic training is enabled
\State build the Hamiltonian $H_t = H(\tilde u_t^{(\mathrm{noisy})}, \eta_t)$
\State propagate the dynamics: $U_{t+1} = e^{-i H_t \Delta t} U_t$
\State update the TSWT-based leakage estimator
\State evaluate the current cost $C_{t+1}$ and reward $r_{t+1}$
\State check the stopping condition (time limit or cost threshold)
\State construct the next observation $o_{t+1}$
\State \Return $(o_{t+1}, r_{t+1}, \mathrm{done}, \mathrm{info})$
\end{algorithmic}
\caption{Interaction between the policy and the quantum-control environment}
\end{algorithm}


\subsection{Implementation of the policy and value networks}
\label{subsec:implementation_policy_value_networks}

The policy and value functions introduced in the previous chapter are implemented here through two separate fully connected feed-forward neural networks, one acting as the stochastic actor and the other as the critic. Their role is not to model the quantum dynamics themselves, which remain entirely specified by the multilevel gmon Hamiltonian and by the UFO objective, but to provide a trainable parametric representation of the control law and of the associated long-term return within the TRPO optimization loop.

At each decision step, the agent receives a state representation built from the current propagator of the truncated multilevel system. As discussed above, each local superconducting mode is truncated to three levels, so that the full physical Hilbert-space dimension is
\begin{equation}
d_{\mathrm{phys}} = 3^2 = 9.
\label{eq:impl_physical_dimension_recalled}
\end{equation}
Let \(U_t \in \mathbb{C}^{9\times 9}\) denote the current full propagator at time step \(t\). The observation is constructed by vectorizing \(U_t\), separating real and imaginary parts, and appending the normalized time index:
\begin{equation}
s_t
=
\Bigl(
...
t/T_{\max}
\Bigr).
\label{eq:impl_state_representation_revised}
\end{equation}
Since \(U_t\) has \(9^2=81\) complex entries, the resulting observation dimension is
\begin{equation}
d_{\mathrm{obs}} = 2\cdot 81 + 1 = 163.
\label{eq:impl_observation_dimension_revised}
\end{equation}

Both actor and critic are implemented as multilayer perceptrons with hidden-layer sizes \((64,32,32)\) and \(\tanh\) activations between successive affine layers, consistently with the architecture reported in the reference paper \cite{Niu2019}. This choice provides a sufficiently expressive nonlinear representation while keeping the number of trainable parameters moderate and the optimization stable.

\paragraph{Policy network.}
The actor maps the current observation \(s_t\) to the parameters of a Gaussian distribution in a seven-dimensional raw-action space. More precisely, the network outputs a mean vector
\begin{equation}
\mu_{\theta}(s_t) \in \mathbb{R}^{7},
\label{eq:impl_policy_mean_revised}
\end{equation}
while exploration is controlled by a diagonal covariance matrix
\begin{equation}
\Sigma_{\theta} = \mathrm{diag}(\sigma_1^2,\dots,\sigma_7^2).
\label{eq:impl_policy_covariance_revised}
\end{equation}
In the present implementation, the logarithm of the standard deviation is parameterized through a state-independent trainable vector,
\begin{equation}
\log \sigma \in \mathbb{R}^{7},
\label{eq:impl_logstd_vector}
\end{equation}
so that the raw action is sampled as
\begin{equation}
\tilde a_t
=
\mu_{\theta}(s_t)
+
\sigma \odot \varepsilon_t,
\qquad
\varepsilon_t \sim \mathcal{N}(0,I_7).
\label{eq:impl_raw_action_sampling_revised}
\end{equation}
where \(\odot\) denotes the Hadamard product.\footnote{The symbol \(\odot\) denotes the Hadamard, or element-wise, product between two vectors of the same dimension. More explicitly, if \(x,y\in\mathbb{R}^n\), then
\(
x\odot y = (x_1 y_1,\dots,x_n y_n).
\)
In Eq.~\eqref{eq:impl_raw_action_sampling_revised}, this means that each component of the noise vector \(\varepsilon_t\) is scaled by the corresponding standard deviation component of \(\sigma\).}


This parameterization deserves a brief clarification. The quantity \(\log \sigma\) denotes the initial value assigned to the vector in Eq.~\eqref{eq:impl_logstd_vector}. It therefore controls the initial exploration scale of the stochastic policy before training refines it. For example, \(\log \sigma = -1\) corresponds to an initial standard deviation \(e^{-1}\approx 0.37\) for each raw-action component. Larger values produce broader exploration, which may help avoid premature convergence but can also make training noisier and less stable; smaller values induce more conservative exploration, which may improve stability but risk insufficient coverage of the control space. In this sense, \(\log \sigma\) is best interpreted as an exploration hyperparameter of the actor, rather than as a performance metric.

The sampled raw action \(\tilde a_t\) is then mapped to a bounded domain through a componentwise hyperbolic tangent,
\begin{equation}
a_t = \tanh(\tilde a_t),
\label{eq:impl_action_squashing_revised}
\end{equation}
so that the effective action always lies in \([-1,1]^7\). This bounded representation is convenient because the physical control channels are amplitude-limited. The normalized action is subsequently rescaled to the actual Hamiltonian controls,
\begin{equation}
u_t
=
\bigl(
g_t,\,
\delta_{1,t},\delta_{2,t},\,
f_{1,t},f_{2,t},\,
\phi_{1,t},\phi_{2,t}
\bigr),
\label{eq:impl_physical_control_vector_revised}
\end{equation}
where the linear-amplitude channels are mapped to their corresponding hardware bounds, while the phase components are mapped from \([-1,1]\) to \([0,2\pi]\). The policy network should therefore be interpreted as a stochastic sequential control law: it does not output the full pulse in one shot, but generates the next control increment conditioned on the current state of the gate-construction process.

\paragraph{Value network.}
The critic is implemented as a second multilayer perceptron with the same hidden-layer structure, but with scalar output
\begin{equation}
V_{\omega}(s_t)\in\mathbb{R},
\label{eq:impl_value_output_revised}
\end{equation}
which approximates the expected discounted future reward from the current state. Its role is not to define the control objective, but to provide a baseline for advantage estimation and thereby reduce the variance of the policy-gradient update. In actor--critic terms, the policy network proposes candidate control actions, while the value network evaluates how promising the current state is under the present policy.

The distinction between the two networks should therefore remain conceptually sharp. The actor produces the parameters of the action distribution and governs exploration in control space; the critic estimates long-term return and supports stable optimization. Neither of them replaces the physical model or the UFO objective. Rather, both are embedded in the reinforcement-learning loop as trainable approximators that make continuous-action policy optimization feasible in the present multilevel quantum-control setting.

For clarity, the hyperparameters that govern the optimization procedure itself---such as the trust-region radius, discount factor, generalized advantage estimation parameters, batch size, and value-function fitting settings---are discussed separately in the training-configuration subsection, since they belong to the TRPO optimization scheme rather than to the architectural definition of the actor and critic.
...
\end{enumerate}
The overall training workflow used for the single-target TRPO controller is explicitly reported in Appendix~\ref{app:trpo_single_target_detailed}.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/trpo_trust_region.pdf}
% \begin{tikzpicture}[
%     box/.style={draw, rounded corners, minimum width=3.2cm, minimum height=1cm, align=center},
%     arrow/.style={-{Latex[length=2mm]}, thick},
%     node distance=1.8cm and 1.8cm
...
% \begin{algorithm}[t]
% \caption{Single-target TRPO training workflow}
% \label{alg:trpo_single_target_compact}
% \begin{algorithmic}[1]
% \Require target gate parameters $(\alpha,\gamma)$, training environment, evaluation environment
% \State initialize the TRPO agent
% \For{each training iteration}
%     \State collect a batch of stochastic rollouts
%     \State store transitions and terminal episode statistics
%     \State update the policy and value networks with TRPO
%     \State summarize batch-level training statistics
%     \If{checkpoint saving is enabled at this iteration}
%         \State save the current agent state
%     \EndIf
%     \If{deterministic evaluation is scheduled at this iteration}
%         \State evaluate the current policy in the nominal environment
%         \State save the corresponding control plan
%         \If{the evaluation cost improves over the best previous one}
%             \State update the best agent and best control plan
%         \EndIf
%     \EndIf
%     \State log the iteration record
% \EndFor
% \State save the final agent and the final evaluated control plan
% \State write a summary of the best and final results
% \end{algorithmic}
% \end{algorithm}
...
A distinction that will be used repeatedly in the remainder of the thesis is that between \emph{nominal} and \emph{noise-optimized} RL training. In the nominal setting, the agent is trained in the deterministic environment, that is, without stochastic perturbations injected into the control channels during training. In the noise-optimized setting, instead, the same actor--critic architecture, the same TRPO update rule, and the same physical control model are retained, but the environment becomes stochastic, since sampled perturbations are added to the relevant Hamiltonian parameters at each control step. The importance of this distinction is conceptual rather than architectural: the two controllers do not correspond to different learning frameworks, but to the same framework trained under different distributions of dynamical trajectories. Stating this point explicitly is useful because it clarifies that the subsequent comparison between nominal and noise-optimized agents is meant to isolate the effect of stochastic training on robustness, rather than the effect of changing the policy model or the optimization algorithm.

At the implementation level, this expected noisy objective is sampled through repeated stochastic rollouts, in which the policy output is filtered, perturbed, and then propagated through the multilevel dynamics, as summarized in Algorithm~\ref{alg:stochastic_rollout_compact}, while a more explicit version, closer to the actual implementation and including the treatment of noisy control channels, phase wrapping, and stopping conditions, is reported in Appendix~\ref{app:stochastic_rollout}.

\begin{algorithm}[t]
\caption{Sampling a rollout in the stochastic training environment}
\label{alg:stochastic_rollout_compact}
\begin{algorithmic}[1]
\Require policy $\pi_\theta$, initial unitary $U_0=\mathbb{I}$, target gate $U_{\mathrm{target}}$
\State reset the environment, the bandwidth filter, and the leakage estimator
\For{$k=0,1,\dots$ until termination}
    \State construct the observation $o_k$ from the current propagated unitary
    \State sample a policy action $a_k \sim \pi_\theta(\cdot \mid o_k)$
    \State map $a_k$ to physical controls and apply the bandwidth filter
    \If{noise-optimized training is enabled}
        \State sample Gaussian perturbations for the relevant control channels and for the effective anharmonicity
        \State build the noisy controls used at this step
    \Else
        \State use the filtered controls without perturbation
    \EndIf
    \State construct the one-step Hamiltonian and propagate
    \State update the TSWT-based leakage estimate
    \State evaluate the current UFO cost and corresponding reward
    \State store the transition $(o_k,a_k,r_{k+1},o_{k+1})$
    \State check the stopping condition
\EndFor
\State \Return the rollout used for policy-gradient training
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{chapters/plots/stochastic_training_environment.pdf}
% \begin{tikzpicture}[
...
\subsection{Implementation of the direct gradient-based baseline}
\label{subsec:implementation_adam_baseline}

Before discussing the comparison strategy in more detail, it is useful to state explicitly how the main non-RL baseline used in this thesis is implemented. This point deserves a dedicated subsection because the baseline is not intended as a minimal or purely heuristic comparator. On the contrary, it is designed as a strong direct pulse-optimization reference, built on the same multilevel physical model and on the same physically structured objective used throughout the rest of the empirical study. In methodological terms, the role of this baseline is to test whether a classical gradient-based optimization of the control trajectory itself is already sufficient to produce high-quality solutions, before introducing the additional machinery of policy-gradient reinforcement learning.

Conceptually, the procedure belongs to the family of direct gradient-based quantum optimal-control methods discussed in subsection~\ref{subsec:classical_qoc_methods}. In particular, it is close in spirit to GRAPE, in the sense that the control field is discretized in time and the optimization is performed directly over the pulse parameters of a fixed target gate \cite{Khaneja2005,Koch2022}. At the same time, the present implementation is not meant as a strict textbook reproduction of the canonical GRAPE algorithm. Rather, it should be understood as a GRAPE-style direct pulse-search baseline in which the discretized controls are optimized numerically with Adam under the same multilevel Hamiltonian and the same UFO-inspired cost structure used elsewhere in the thesis. This distinction is worth stating explicitly, because it clarifies both the methodological continuity with classical optimal control and the specific implementation choices adopted here.

For a fixed target gate \(U_{\mathrm{target}} = N(\alpha,\alpha,\gamma)\) and a fixed candidate gate duration \(T\), the control problem is discretized on a uniform time grid of step \(\Delta t\), so that
\(
N_T = \frac{T}{\Delta t}
\)
time slices are used. The optimization variables are then the full time-discretized control trajectory
\begin{equation}
\Bigl\{
g_\ell,\,
\delta_{1,\ell},\delta_{2,\ell},\,
f_{1,\ell},f_{2,\ell},\,
\phi_{1,\ell},\phi_{2,\ell}
\Bigr\}_{\ell=0}^{N_T-1},
\label{eq:adam_discretized_controls}
\end{equation}
which parameterizes the effective Hamiltonian over the entire pulse. In other words, unlike the RL controller, which learns a stochastic sequential policy over actions, the direct baseline optimizes the control amplitudes themselves as explicit free parameters of a fixed-horizon pulse.

For each such discretized pulse, the physical evolution is simulated on the same truncated multilevel gmon model introduced previously, and the corresponding control quality is evaluated through the same cost-function structure that underlies the UFO framework, described in subsection~\ref{subsec:ufo_cost_function}.  This choice is important for the interpretation of the later comparisons. It means that the Adam baseline is not solving a different or artificially simplified control problem; it is solving essentially the same physical optimization problem as the RL controller, but through a direct search in pulse space rather than through policy optimization.

The numerical optimization is then carried out independently for each candidate gate duration using the Adam optimizer. Denoting collectively by \(\bm{c}_T\) the discretized control variables associated with horizon \(T\), the optimization problem may be written schematically as
\begin{equation}
\bm{c}_T^{\star}
=
\arg\min_{\bm{c}_T}
C(\bm{c}_T;T),
\label{eq:adam_fixed_horizon_problem}
\end{equation}
where the dependence on \(T\) is explicit both through the number of time slices and through the runtime penalty entering the objective. In practice, the optimizer is run for a fixed number of Adam iterations at each candidate horizon, producing one locally optimized control plan per duration.

A further methodological feature of the baseline is that the gate duration is not optimized continuously inside a single run. Instead, the problem is solved repeatedly over a discrete set of candidate horizons,
\begin{equation}
T \in \mathcal{T}_{\mathrm{cand}},
\label{eq:adam_candidate_horizons}
\end{equation}
and the resulting horizon-wise optimized solutions are then compared. This means that the Adam baseline should be interpreted as a \emph{family} of fixed-horizon pulse-optimization problems rather than as a single variable-time control policy. Such a design is fully reasonable in the present context. It allows the direct optimizer to search efficiently within each fixed temporal budget, while still making it possible to identify which runtime provides the best overall compromise among fidelity, leakage, boundary behavior, and total cost.

Once all candidate horizons have been optimized, the corresponding solutions are reevaluated in a common deterministic setting using the same multilevel simulator. This post-optimization reevaluation is an important part of the pipeline, because it separates the internal optimizer output from the final reported physical metrics. In particular, for each candidate horizon the thesis records not only the optimizer-side best objective value, but also a full nominal evaluation including final cost, gate fidelity, total leakage, leakage decomposition, boundary penalty, and effective runtime. The best horizon is then selected on the basis of this systematic comparison, and the associated control plan is stored as the representative Adam baseline solution for the considered target gate.
The overall workflow used for the single-target Adam baseline is summarized in Algorithm~\ref{alg:adam_baseline_horizon_sweep}.

When required (through the \texttt{noise-optimized} argument is set to true), the selected plan is further subjected to Monte Carlo robustness evaluation under sampled control perturbations. In this way, the baseline participates in the same robustness analysis as the RL-based solutions, rather than being restricted to purely nominal benchmarking. This is methodologically important, because it ensures that the later comparison between Adam and TRPO is not a comparison between a fully evaluated learning-based controller and a weakly characterized deterministic baseline. On the contrary, both methods are assessed under the same physical criteria, including noisy deployment when robustness is under study.

Taken together, this implementation makes the direct gradient-based baseline a strong and scientifically meaningful reference. It is strong because it optimizes the full pulse trajectory directly on the multilevel Hamiltonian, rather than relying on a simplified gate-level abstraction. It is fair because it uses the same target gates, the same physical model, and the same cost-function logic as the RL framework. And it is informative because its structure is genuinely different from that of TRPO: the Adam baseline performs direct fixed-horizon pulse optimization in control space, whereas TRPO learns a stochastic sequential policy that implicitly balances control shape and stopping behavior. Precisely because of this difference, the comparison between the two approaches is methodologically useful. It does not ask whether one optimizer is universally better in the abstract, but whether policy-based learning offers an advantage over a strong direct pulse-search strategy on the same underlying analog quantum-control problem.

\begin{algorithm}[H]
\caption{Direct gradient-based baseline with horizon sweep}
\label{alg:adam_baseline_horizon_sweep}
\begin{algorithmic}[1]
\Require Target gate \(U_{\mathrm{target}}=N(\alpha,\alpha,\gamma)\), candidate horizons \(\mathcal{T}_{\mathrm{cand}}\), timestep \(\Delta t\), cost weights \((\chi,\beta,\mu,\kappa)\)
\For{each \(T \in \mathcal{T}_{\mathrm{cand}}\)}
    \State Discretize the control trajectory into \(N_T = T/\Delta t\) time steps
    \State Initialize the pulse parameters \(\bm{c}_T\)
    \For{\(m=1,\dots,M_{\mathrm{Adam}}\)}
        \State Simulate the multilevel dynamics under \(\bm{c}_T\)
        \State Compute the objective \(C(\bm{c}_T;T)\)
        \State Update \(\bm{c}_T\) with Adam
    \EndFor
    \State Store the optimized plan \(\bm{c}_T^{\star}\)
    \State Reevaluate nominally: cost, fidelity, leakage, boundary penalty, runtime
\EndFor
\State Select the best horizon according to the reevaluated performance
\State Optionally evaluate robustness through noisy Monte Carlo rollouts
\State \Return Best control plan and associated metrics
\end{algorithmic}
\end{algorithm}

%\textbf{TODO:} Da rileggere domani

\subsection{Baselines and comparison strategy}
\label{subsec:baselines_comparison_strategy}

The first baseline is the direct gradient-based pulse-optimization method described in subsection~\ref{subsec:implementation_adam_baseline}. It represents the strongest non-RL comparator considered in this thesis, since it optimizes the full discretized control trajectory directly on the same multilevel physical model and under the same UFO-inspired cost structure.
...
The first baseline is a direct optimization of the control trajectory using the GRAPE method, implemented with the Adam optimizer, and an instance of Stochastic Gradient Descent (SGD) \cite{Niu2019,Kingma2015}. This baseline is important because it represents the most immediate non-RL alternative: one keeps the same physical control problem and the same general objective structure, but optimizes the control parameters directly rather than through a policy-gradient agent. In the logic of the paper, this baseline tests whether the use of a policy-based second-order learning method such as TRPO actually provides an advantage over simpler first-order optimization of the raw control variables.
...
% %\textbf{TODO:} da sistemare 
% FOR NOW AI GENERATED THIS SECTION (TO BE INCLUDED THE PAPERS ORIGINAL RESULTS)
...
The primary finding concerns the performance of the noise-optimized RL controller in a stochastic environment. The results of this investigation are reported in Table~\ref{tab:robustness_results_summary}.
...
\hline
Tested noise range & $\sigma_{\mathrm{noise}} \in [0.1, 3.5]$ MHz & Sweep of standard deviations \\
Average gate fidelity & $\overline{F}_{\mathrm{RL,noisy}} \in [99.5\%, 98\%]$ & Noisy performance regime \\
Representative noise & $\sigma_{\mathrm{noise}} = 1$ MHz & Experimental reference scale \\
Robustness condition & $1-\overline{F} \le \epsilon_0, \;\epsilon_0 = 0.007$ & Robustness criterion satisfaction \\
\hline
\end{tabular}
\vspace{0.5em}
\caption{Assessing the robustness performance of the noise-trained policy. The table summarizes the key benchmarks and performance thresholds across different noise regimes as reported in \cite{Niu2019}.}
\label{tab:robustness_results_summary}
\end{table}
...
\emph{one order of magnitude} improvement in average gate infidelity relative to the RL controller trained without stochastic noise, and around \emph{two orders of magnitude} improvement in average gate infidelity relative to the SGD baseline \cite{Niu2019}. This is one of the most important conclusions of the paper, because it isolates the source of the improvement: the gain is not due only to the use of RL in general, but specifically to the combination of policy-gradient learning with a noisy training environment.
...
%\textbf{TODO:} Aggiungere un diagramma parlante totale su environment fisico